...
public:
	...
	void onDestroy();
	bool update(const float deltaTime_, bool foward_sequence = true);
	bool updateOnce(const float deltaTime_);
	void input();
	...
	void draw(std::shared_ptr<Shader_Actor> shader, glm::mat4 projection, float xpos, float ypos, float boxWidth, float bowHeight, float rot, glm::vec3 colour) const;
