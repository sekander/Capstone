...
Texture_Actor::Texture_Actor(Component* parent_) : Actor(parent_)
{
	...
	//ListComponents();
	ObjectCounter::texture_actor_counter++;
...
void Texture_Actor::draw(std::shared_ptr<Shader_Actor> shader,  glm::mat4 projection, float xpos, float ypos, float boxWidth, float boxHeight, float rot,
glm::vec3 colour) const
{
	shader->GetComponent<ShaderComponent>()->Use();
    		...
    		shader->GetComponent<ShaderComponent>()->SetMatrix4("model", GetComponent<TransformComponent>()->transform(
					vec2(xpos, ypos), 
					vec2(boxWidth, boxHeight), rot));
    		shader->GetComponent<ShaderComponent>()->SetVector3f("color", colour);
    GetComponent<RenderComponent>()->drawSprites();
}
