...
void Text_Actor::draw(glm::mat4 projection, std::string text, float xpos, float ypos, float r, float g,float b) const
{
	GetComponent<ShaderComponent>()->SetInteger("text", 0);
	GetComponent<ShaderComponent>()->Use();
  		...
  		glm::mat4 model;
  		model = glm::mat4(1.0f);
  		GetComponent<TextRenderComponent>()->RenderText(text, xpos, ypos, 0.5f);
